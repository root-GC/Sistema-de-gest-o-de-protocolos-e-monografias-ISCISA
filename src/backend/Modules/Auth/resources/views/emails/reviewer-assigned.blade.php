{{-- Modules/Reviews/resources/views/emails/assigned-reviewer.blade.php --}}
<!DOCTYPE html>
<html lang="pt">
<head>
    <meta charset="utf-8">
    <title>Atribuido como revisor</title>
</head>
        ...
        <tr>
            <td>
                ...
                <p style="color:#333;font-size:15px;">
                    Foi atribuido como <strong>avaliador do {{ $entityType }}</strong> abaixo:
                </p>
                <div style="background:#f0f2f5;border-radius:6px;padding:16px;margin:16px 0;">
                    <p style="margin:0 0 4px;font-size:13px;color:#666;">{{ $entityType === 'tema' ? 'Titulo' : 'Protocolo' }}</p>
                    <p style="margin:0 0 12px;font-size:16px;color:#1a1a1a;font-weight:bold;">{{ $entityTitle }}</p>
                    @if ($studentName)
                        <p style="margin:0 0 4px;font-size:13px;color:#666;">Estudante</p>
                        <p style="margin:0 0 12px;font-size:14px;color:#333;">{{ $studentName }}</p>
                    @endif
                    @if ($course)
                        <p style="margin:0 0 4px;font-size:13px;color:#666;">Curso</p>
                        <p style="margin:0;font-size:14px;color:#333;">{{ $course }}</p>
                    @endif
                </div>
