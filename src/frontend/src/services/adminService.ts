...
// 🆕 Interfaces para estado do sistema
...
export const adminService = {
  ...
  createScientificArea: (data: { organ_id: number; name: string; description?: string }) =>
    req('POST', '/api/v1/admin/scientific-areas', data) as Promise<{ message: string; scientific_area: ScientificArea }>,

  updateScientificArea: (id: number, data: { organ_id?: number; name?: string; description?: string }) =>
  ...
  updateCourse: (id: number, data: { scientific_area_id?: number; name?: string; code?: string; description?: string }) =>
  ...
  getSystemMetrics: () =>
    req('GET', '/api/v1/admin/system/metrics') as Promise<{ data: SystemMetrics }>,

  getSystemServices: () =>
    req('GET', '/api/v1/admin/system/services') as Promise<{ data: ServiceStatus[] }>,
  ...
  // ── 🆕 System Statistics ──────────────────────────────────
  getSystemStats: () =>
    req('GET', '/api/v1/admin/system/stats') as Promise<{ data: SystemStats }>,

  getProtocolsByStatus: () =>
    req('GET', '/api/v1/admin/system/stats/protocols-by-status') as Promise<{ data: Record<string, number> }>,

  getTopicsByStatus: () =>
    req('GET', '/api/v1/admin/system/stats/topics-by-status') as Promise<{ data: Record<string, number> }>,

  getSubmissionsTrend: (days?: number) => {
    const query = days ? `?days=${days}` : ''
    return req('GET', `/api/v1/admin/system/stats/submissions-trend${query}`) as Promise<{
  ...
  getAuditLogs: (params?: { user_id?: number; action?: string; page?: number; limit?: number }) => {
    ...
    if (params?.limit) query.append('limit', String(params.limit))
    const qs = query.toString()
    return req('GET', `/api/v1/admin/audit-logs${qs ? `?${qs}` : ''}`) as Promise<{
      data: AuditLog[]
      total: number
      current_page: number
    }>
  ...
  clearCache: (tag?: string) =>
    req('POST', '/api/v1/admin/system/cache/clear', { tag }) as Promise<{ message: string }>,

  getCacheStats: () =>
    req('GET', '/api/v1/admin/system/cache/stats') as Promise<{
      data: {
        driver: string
        size_mb: number
        hits: number
  ...
  // ── 🆕 Queue Management ───────────────────────────────────
  getQueueStats: () =>
    req('GET', '/api/v1/admin/system/queue/stats') as Promise<{
      data: {
        pending: number
        processing: number
        completed: number
        failed: number
        failed_jobs: { id: number; queue: string; payload: string; failed_at: string }[]
      }
    }>,

  retryFailedJobs: () =>
    req('POST', '/api/v1/admin/system/queue/retry-failed') as Promise<{ message: string; count: number }>,

  // ── 🆕 Database Info ──────────────────────────────────────
  getDatabaseInfo: () =>
    req('GET', '/api/v1/admin/system/database/info') as Promise<{
      data: {
        driver: string
        version: string
        size_mb: number
  ...
  // ── 🆕 Backup Status ──────────────────────────────────────
  getBackupStatus: () =>
    req('GET', '/api/v1/admin/system/backup/status') as Promise<{
      data: {
  ...
  triggerBackup: () =>
    req('POST', '/api/v1/admin/system/backup/trigger') as Promise<{ message: string }>,
}